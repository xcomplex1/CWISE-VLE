...
View.prototype.retrieveImportProjectListSuccess = function(responseText, responseXML, args) {
	...
	if(projectType == 'authorable') {
		$('#importFromAuthorableProjectList').append('<option name="authorableProjectOption" value="">選擇可編輯專題</option>');			
	} else if(projectType == 'library') {
		$('#importFromLibraryProjectList').append('<option name="libraryProjectOption" value="">選擇專題庫專題</option>');
	}
