...
View.prototype.SurgeNode.generatePage = function(view){
	...
	var levelStringLabel = document.createTextNode("等級字串:");
	...
	var sourceLabel = $(document.createElement('div')).text('SURGE活動原始檔 (swf):');
	//create the radio buttons and labels for each source option
	var sourceLabelDefault = $(document.createElement('span')).text('預設 (一般等級)');
	...
	var sourceLabelCustom = $(document.createElement('span')).text('客製化');
	...
	var swfUrlLabel = $(document.createElement('span')).text('客製化swf檔案:');
	...
	var swfBrowseButton = $(createElement(document, 'button', {id: 'swfBrowseButton', onclick:'eventManager.fire("surgeBrowseClicked")'})).text('瀏覽');
