...
View.prototype.ExplanationBuilderNode.generatePage = function(view){
	...
	var requireWorkText = document.createTextNode("在進行下一步驟以前學生必須完成此步驟：");
	...
	var enableStudentTextAreaText = document.createTextNode("學生回應欄位：");
	...
	var backgroundImageUrlLabel = document.createTextNode("組織空間背景圖片：");
	var maxImageSizeLabel = document.createTextNode(" - 組織空間背景圖片大小： 640x480 pixels");
	...
	var backgroundBrowseButton = $(createElement(document, 'button', {id: 'backgroundBrowseButton', onclick:'eventManager.fire("explanationBuilderBrowseClicked")'})).text('瀏覽');
	
	//the label for the background align drop-down select
	var backgroundAlignLabel = document.createTextNode("背景對齊： ");
	...
	var leftTopOption = $(createElement(document, 'option', {value: 'Left-Top'})).append($(document.createTextNode('左-上')));
	var leftMiddleOption = $(createElement(document, 'option', {value: 'Left-Middle'})).append($(document.createTextNode('左-中')));
	var leftBottomOption = $(createElement(document, 'option', {value: 'Left-Bottom'})).append($(document.createTextNode('左-下')));
	var centerTopOption = $(createElement(document, 'option', {value: 'Center-Top'})).append($(document.createTextNode('中-上')));
	var centerMiddleOption = $(createElement(document, 'option', {value: 'Center-Middle'})).append($(document.createTextNode('中-中')));
	var centerBottomOption = $(createElement(document, 'option', {value: 'Center-Bottom'})).append($(document.createTextNode('中-下')));
	var rightTopOption = $(createElement(document, 'option', {value: 'Right-Top'})).append($(document.createTextNode('右-上')));
	var rightMiddleOption = $(createElement(document, 'option', {value: 'Right-Middle'})).append($(document.createTextNode('右-中')));
	var rightBottomOption = $(createElement(document, 'option', {value: 'Right-Bottom'})).append($(document.createTextNode('右-下')));
