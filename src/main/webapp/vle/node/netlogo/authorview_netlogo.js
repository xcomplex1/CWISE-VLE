...
View.prototype.NetlogoNode.generatePage = function(view){
	...
	var nlogoUrlLabel = $(document.createElement('span')).text('Netlogo (.nlogo) 位址:');
	...
	var nlogoBrowseButton = $(createElement(document, 'button', {id: 'nlogoBrowseButton', onclick:'eventManager.fire("netlogoBrowseClicked")'})).text('瀏覽');
	...
	var nlogoDimensionsLabel = $(document.createElement('div')).text('尺寸 (px):');
	//create the label for the textarea that the author will write the height in
	var nlogoHeightLabel = $(document.createElement('span')).text('高:');
	...
	var nlogoWidthLabel = $(document.createElement('span')).text('寬:');
